...
for (i in temp_i) {
  ...
  if (sum(tempInt == 369) == 0) {} else {
    ...
    tempScript <- paste(tempScript, ifelse(temp_negatives/24 < (temp_positives+1), paste(tempList2, paste(tempList2, ifelse(tempList2 == "", "", ", ", sep = ""), tempList1, sep = ""), paste(tempList1, ", ", i, sep = "")), "), ]\n", sep = "")
    cat(tempScript, sep = "")
    #close(prog_bar)
  }
}
