# TAKE WITH CAUTION AND DO NOT USE IT
# TAKE WITH CAUTION AND DO NOT USE IT
# TAKE WITH CAUTION AND DO NOT USE IT
# WARNING
# WARNING !!!!!!!!!!!!!!!!!!!
# This version might conflict with old version results, TAKE WITH CAUTION AND DO NOT USE IT
...
sink("debloat.R", append = FALSE, split = TRUE)
for (i in temp_i) {
  tempStr <- "\n"
  ...
  #close(prog_bar)
  if (sum(tempInt == 369) == 0 | !(excluded_index[i] == -1)) {} else { #don't allow to parse twice the duplicated positive row
    #cat("\nInspecting ", i, "\n", sep = "")
    ...
    tempScript <- paste(tempScript, ifelse(temp_negatives/24 < (temp_positives+1), paste(tempList2, ifelse(tempList2 == "", "", ", "), tempList1, sep = ""), paste(tempList1, ifelse(tempList1 == "", "", ", "), i, sep = "")), "), ]\n", sep = "")
    cat(tempScript, sep = "")
    #close(prog_bar)
  }
}
